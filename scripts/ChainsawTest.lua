...
function ChainsawTest:update(chainsaw)
    ...
    if chainsaw.ringSelector ~= nil and getVisibility(chainsaw.ringSelector) then
        ...
        local zx,zy,zz = localDirectionToWorld(chainsaw.chainsawSplitShapeFocus, 0,0,1)
        ...
        if shapeId ~= nil and shapeId ~= 0 then
            ...
            Utils.renderTextAtWorldPosition(x,y,z, ('Found shapeId: %d'):format(shapeId), getCorrectTextSize(0.05), 0)
            ...
            zx,zy,zz = localDirectionToWorld(shapeId, 0,0,-1) -- unit vector along the local Y axis of the shape, but in world coordinates
            ...
            if shapeId ~= nil and shapeId ~= 0 then
                local radius = (minY+maxY+minZ+maxZ)/8.0
                Utils.renderTextAtWorldPosition(x,y,z, ('Found shapeId: %d'):format(shapeId), getCorrectTextSize(0.05), 0)

                local rX, rY, rZ = localToWorld(chainsaw.ringSelector, 0,0,0)
                Utils.renderTextAtWorldPosition(rX,rY,rZ, ('Radius: %.3f'):format(radius), getCorrectTextSize(0.05), 0)
                DebugUtil.drawDebugAreaRectangle(
